<?php
/*
Plugin Name:    Javier Package Demo Data
Plugin URI:     http://la-studioweb.com/
Description:    This plugin use only for Javier Theme
Author:         LA Studio
...
if (!defined('ABSPATH') && !defined('JAVIER_OPTION') && !defined('JAVIER_THEME_OPTION')){
    die();
}

defined( 'LA_TEXTDOMAIN' )     or  define( 'LA_TEXTDOMAIN',     'la-studio' );

class Javier_Data_Demo_Plugin_Class{
    ...
    protected function __construct( ) {
        ...
        add_filter('javier/filter/demo_data', array( $this, 'getDemoData' ) );
        add_filter('body_class', array( $this, 'bodyClass' ) );
        add_shortcode('la_select_demo', array( $this, 'registerShortcodeDemoSelect'));

        add_filter('javier/filter/getOption', array( $this, 'overrideThemeOptions' ), 11, 3 );
...
//
    ...
    public function registerMenu(){
        if(class_exists('CSFramework')){
            require plugin_dir_path( __FILE__ ) . 'panel.php';
        }
    }
    ...
    protected function overrideFilterByQueryString(){
        $allow = array(
            'getOptionByMetadata' => array(
                'name'  => 'javier/filter/getOptionByMetadata',
                'key'   => '',
                'value' => array(
                    25
                )
    ...
    public function overrideThemeOptions( $options, $option_name, $default ){
        ...
        $global_options = isset($GLOBALS['javier_theme_options']) && !empty($GLOBALS['javier_theme_options']) ? $GLOBALS['javier_theme_options'] : $options;
        ...
        if(isset($_GET['_preset']) && !empty($_GET['_preset'])){
            ...
            $GLOBALS['javier_theme_options'] = $global_options;
        }
    ...
    public function bodyClass($classes){
        if(is_page() && !empty($GLOBALS['javier_theme_options']['body_class'])){
            $classes[] = esc_attr($GLOBALS['javier_theme_options']['body_class']);
        }
        return $classes;
    }
